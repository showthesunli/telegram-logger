...
class MentionReplyHandler(BaseHandler):
    ...
    def __init__(
        ...
        **kwargs: Dict[str, Any],
        ...
        logger.info(
            "MentionReplyHandler 初始化完成。"
        )  # 移除 my_id 的日志记录，因为它此时尚未设置
    ...
    async def handle_event(self, event: events.NewMessage.Event):
        ...
        try:  # 添加顶层 try 块
            ...
            logger.debug(
                f"Handling event in ChatID={event.chat_id}. Reply trigger status: {reply_trigger_status}, Mentioned status: {mention_status}, IsReply: {event.is_reply}"
            )
            # --- 调试日志结束 ---

            logger.debug(
                f"MentionReplyHandler 收到事件: ChatID={event.chat_id}, MsgID={event.id}, SenderID={event.sender_id}"
            )
            ...
            logger.debug(
                f"事件消息是{event.message.text}，提及: {is_mention}, 回复: {is_reply}"
            )

            if not is_mention and not is_reply:
                # --- 新增日志 ---
                logger.debug(
                    "条件 (not is_mention and not is_reply_to_me) 为 True，准备忽略并返回。"
                )
                # --- 日志结束 ---
                logger.debug(
                    "事件既不是 @提及 也不是对我的回复（或回复触发未启用），忽略。"
                )
                return

            if is_reply and is_reply_trigger_enabled:
                logger.debug("事件是回复，但是回复触发未启用，准备忽略。")
                return

            # --- 新增日志 ---
            logger.debug(
                "条件 (not is_mention and not is_reply_to_me) 为 False，继续执行回复流程。"
            )
            # --- 日志结束 ---
