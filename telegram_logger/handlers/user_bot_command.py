    ...
    async def handle_command(self, event: events.NewMessage.Event):
        ...
        try:
            ...
            # --- 其他指令的占位符 ---
                ...
                if args:
                    await self._safe_respond(event, "错误：`.help` 指令不需要参数。")
                    return
...
🔹 `.on` - 启用自动回复功能。
🔹 `.off` - 禁用自动回复功能。
🔹 `.status` - 查看当前状态 (启用/禁用, 回复触发, 模型, 角色, 历史数量, 目标群组, 频率限制)。

**触发控制:**
🔹 `.replyon` - 启用对回复您消息的自动触发。
🔹 `.replyoff` - 禁用对回复您消息的自动触发 (仅 @ 提及有效)。

**AI 模型管理:**
🔹 `.setmodel <模型ID或别名>` - 选择当前使用的 AI 模型。
🔹 `.listmodels` - 列出可用的模型别名及其对应的模型 ID。
🔹 `.aliasmodel <模型ID> <别名>` - 为模型 ID 设置别名。
🔹 `.unaliasmodel <别名>` - 删除模型别名。
🔹 `.sethistory <数量>` - 设置 AI 回复时加载的历史消息数量 (0-20)。

**AI 角色管理:**
🔹 `.setrole <别名>` - 选择当前使用的角色别名。
🔹 `.listroles` - 列出所有已定义的角色别名及其配置。
🔹 `.aliasrole <别名> --type ai` - 创建 AI 类型的角色别名。
🔹 `.aliasrole <别名> "<静态内容>" --type static` - 创建 Static 类型的角色别名并设置内容。
🔹 `.setroledesc <别名> "<描述>"` - 设置角色描述。
🔹 `.setroleprompt <别名> "<系统提示>"` - 设置 AI 角色的系统提示。
🔹 `.setrolepreset <别名> '<JSON列表>'` - 设置 AI 角色的预设消息 (Few-shot)。
🔹 `.unaliasrole <别名>` - 删除角色别名及其配置。

**目标群组管理:**
🔹 `.addgroup <群组ID或链接>` - 添加自动回复生效的目标群组。
🔹 `.delgroup <群组ID或链接>` - 移除目标群组。
🔹 `.listgroups` - 列出当前所有目标群组。

**频率限制:**
🔹 `.setlimit <秒数>` - 设置在同一群组内触发自动回复的最小间隔。

**帮助:**
🔹 `.help` - 显示此帮助信息。
"""
                await self._safe_respond(event, help_message.strip())
